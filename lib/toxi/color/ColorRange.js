define([
    './TColor',
    './ColorList',
    '../utils/datatypes/FloatRange',
    '../internals',
    '../math/mathUtils'
],function( TColor, ColorList, FloatRange, internals, MathUtils ){

    var ColorRange,
        isColorList = internals.tests.isColorList,
        isColorRange = internals.tests.isColorRange,
        isFloatRange = internals.tests.isFloatRange,
        isHue = internals.tests.isHue,
        isTColor = internals.tests.isTColor,
        each = internals.each,
        UNTITLED_ID = 1,
        addRange,
        addAll,
    ...
    addRange = function(attr){
        return function( min, max ){
            this[attr.toLowerCase()+'Constraint'].push( isFloatRange(min) ? min : new FloatRange(min,max) );
            return this;
        };
    };
    ...
    * 2.
        ...
        * @param {toxi.utils.datatypes.FloatRange} [white]
        * @param {String} [name]
    * 3.
        * @param {toxi.util.datatypes.FloatRange} [hue]
        * @param {toxi.util.datatypes.FloatRange} [sat]
    ...
    */
    ColorRange = function( hue, sat, bri, alpha, black, white, name ){
        ...
        this.hueConstraint = [isFloatRange(hue) ? hue : new FloatRange(0,1)];
        this.saturationConstraint = [isFloatRange(sat) ? sat : new FloatRange(0,1)];
        this.brightnessConstraint = [isFloatRange(bri) ? bri : new FloatRange(0,1)];
        this.alphaConstraint = [isFloatRange(alpha) ? alpha : new FloatRange(1,1)];
        //not arrays for black & white
        this.black = isFloatRange(black) ? black : new FloatRange(0,1);
        this.white = isFloatRange(white) ? white : new FloatRange(0,1);
