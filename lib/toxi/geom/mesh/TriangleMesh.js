define([
	...
	function vertexKeyGenerator( v ){
		...
		return Vec3D.prototype.toString.call( v );
	}
	...
	var	TriangleMesh = function(name){
	...
	TriangleMesh.prototype = {
		...
		//center must be an async method for getBoundingBox
		//if no function is called it is assumed the response is not needed
		center: function(origin, fn ){
		...
		toWEMesh: function( callback ) {
			var self = this;
			if( callback === undefined ){ throw Error("toWEMesh is asynchronous, pass a callback to receive the mesh"); }
			require(['./WETriangleMesh'], function( WETriangleMesh ){
				callback( new WETriangleMesh(this.name).addMesh(self) );
			});
		},
