...
TriangleMesh.prototype = {
	...
	createVertex: function(vec3D,id){
		var vertex = new Vertex( vec3D, id );
		//hold a map that allows the vector to be used to find its vertex
	...
	/*  return new WETriangleMesh(name, vertices.size(), faces.size())
				.addMesh(this);
	*/
		throw Error("TriangleMesh.toWEMesh() currently not implemented");
	},
	...
	*/
	...
	updateVertex: function(orig,newPos) {
		var vertex = this.__vectorMap[ orig ];
		if (vertex !== undefined ) {
			this.__vectorMap[ orig ] = undefined;
			vertex.set( newPos );
			this.__vectorMap[ newPos ] = vertex;
		}
		return this;
	}
};
