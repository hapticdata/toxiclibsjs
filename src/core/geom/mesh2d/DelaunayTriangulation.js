(function(){
	var DelaunayTriangulation = function(triangle){
		this.__triGraph = new toxi.UndirectedGraph();
		this.__triGraph.add(triangle);
		this.__mostRecent = triangle;
	};

	toxi.extend(DelaunayTriangulation,toxi.ArraySet);

	toxi._.mixin(DelaunayTriangulation.prototype,{
		...
		iterator: function(){
			return new toxi._.Iterator(this.__triGraph.getNodes());
		},
		...
		__update: function(site, cavity){
			var self = this,
				boundary = new toxi.ArraySet(),
				theTriangles = new toxi.ArraySet(),
				triangle;
			...
			var newTriangles = new toxi.ArraySet();
			for(var b=0,blen = boundary.length;b<blen;b++){
				var vertices = boundary[b];
				vertices.push(site);
				var tri = new toxi.DelaunayTriangle(vertices);
	...
	toxi.DelaunayTriangulation = DelaunayTriangulation;
}());