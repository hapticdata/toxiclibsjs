...
(function(){
	...
	toxi.extend(DelaunayTriangle,toxi.ArraySet);
	toxi.DelaunayTriangle = DelaunayTriangle;

	
	toxi._.mixin(DelaunayTriangle.prototype,{
		...
		getCircumcenter: function(){
			if(this.__circumcenter === undefined){
				this.__circumcenter = toxi.DelaunayVertex.circumcenter(this);
			}
			return this.__circumcenter;
		},
		getVertexButNot: function(badVertices){
			if(arguments.length == 1 && toxi._.isArray(badVertices)){
		...
		iterator: function(){
			return new toxi._.Iterator(this);
		},
		toString: function(){
			if(!moreInfo){
