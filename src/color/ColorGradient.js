(function(){
	...
	_GradPoint.prototype = {
		compareTo: function(p){
			if(toxi._.numberCompare(p._pos,this._pos) == 0){
				return 0;
			}
	...
	var ColorGradient = function(){
		this._gradient = [];
		this._interpolator = new toxi.LinearInterpolation();
	};
	...
	ColorGradient.prototype = {
		...
		*/
		setMaxDither: function(maxDither){
			this._maxDither = toxi.MathUtils.clip(this._maxDither, 0, 1);
		}
	};
	
	toxi.color.ColorGradient = ColorGradient;
}())