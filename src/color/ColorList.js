(function(){
	...
	var	_each = function(cl,fn){
 		toxi._.each(cl._colors,fn);
 	};
	...
	*/
	...
	ColorList.protoype = {
		...
		*/
		addAll:	function(collection){
			toxi._.each(collection,function(color){
		...
		*/
		adjustBrightness: function(step){
			toxi._.each(this._colors,function(c){
				c.lighten(step);
			});
			return this;
		},
		...
		*/
		getAverage: function(){
			...
			if(num > 0){
				return toxi.color.TColor.newRGBA(r / num, g / num, b / num, a / num);
			} 
			return undefined;
		},
		/**
		...
		*/
		...
		getRandom: function(){
			var index = Math.floor(toxi.MathUtils.random(this._colors.length));
		...
		iterator: function(){
			return new toxi._.Iterator(this._colors);
		},
		...
		rotateRYB: function(theta, isRadians){
			...
			if(theta !== Math.floor(theta) || isRadians){
				angle = toxi.MathUtils.degrees(theta);
			} else {
				angle = theta;
			}
			_each(function(c){
		...
		sort: function(){
			return this.sortByCriteria(toxi.color.AccessCriteria.HUE, false);
		},
		/**
		...
		*/
		...
		sortByDistance: function(proxy, isReversed){
			if(arguemnts.length == 1){
				...
				proxy = new toxi.color.HSVDistanceProxy();
			}
		...
		*/
		sortByProximityTo: function(target, proxy, isReversed){
			if(arguments.length == 2){
				target = arguments[0];
				proxy = new toxi.color.RGBDistanceProxy();
				isReversed = arguments[1];
			}
			return this.sortByComparator(new toxi.color.ProximityComparator(target,proxy), isReversed);
		},
	 ...
	 */
	 ColorList.createFromARGBArray = function(pixels, num, uniqueOnly, maxIterations){
	 	...
	 	num = toxi.MathUtils.min(num, pixels.length);
	 	if(!uniqueOnly && num == pixels.length){
	 		return new ColorList(pixels);
	 	}

	 	var colors = [],
	 		temp = toxi.color.TColor.BLACK.copy(),
	 		i = 0,
	 		isUnique = true,
	 		numTries = 0,
	 		idx;
	 	for(i=0;i<num;i++){
	 		if(uniqueOnly){
	 			...
	 			numTries = 0;
	 			do {
	 				idx = toxi.MathUtils.random(pixels.length);
	 			...
	 			idx = toxi.MathUtils.random(pixels.length);
	 			colors.push(toxi.color.TColor.newARGB(pixels[idx]));
	 		}
	 	}
	...
	*/
	ColorList.createUsingStrategy = function(strategy, c){
		if(typeof strategy == 'string'){
			strategy = toxi.color.ColorTheoryRegistry.getStrategyForName(strategy);
		}
		var list;
	...
	toxi.color.ColorList = ColorList;
}())